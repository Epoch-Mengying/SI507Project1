...
class Song(object):
    ...
    def __str__(self):
        return "{} by {} on {} whose URL is {}".format(self.track,
                                                       self.artist,
                                                       self.album,
                                                       self.track_url)
    ...
    def num_words_in_name_sans_determiners(self):
        return len([x for x in self.album.split() if (x.lower() != "the") and
                    (x.lower() != "a")])
...
def random_song(list_of_song_objs):
    ...
    print(song_picked.track, song_picked.artist)  # just return track name and
    # artist tuple for identification

...
class Deck(object):
    ...
    def __str__(self):
        ...
        return "\n".join(total)  # returns a multi-line string with each card

    def pop_card(self, i=-1):
        return self.cards.pop(i)  # this card is no longer in the deck
...
def show_song(inp="Winner"):  # default winner ... but also could be something
    # else if it works correctly, which it does not (put in description and
    # remove this clarity)
    songs_resp = helper_functions.get_and_cache_songs(random.choice(["win",
                                                                     "winner",
                                                                     "hurrah",
                                                                     "hooray"])
                                                      )
